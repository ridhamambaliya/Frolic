...
export const getMyInstituteController = async (req, res, next) => {
  try {
    const institute = await getMyInstituteService(req.user._id);

    res.status(200).json({
      success: true,
      message: "My institute fetched successfully",
      data: institute,
    });
  } catch (error) {
    next(error);
