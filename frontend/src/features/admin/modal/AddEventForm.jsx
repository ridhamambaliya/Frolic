  ...
  useEffect(() => {
    const fetchMeta = async () => {
      try {
        ...
        setCoordinators(
          userData.filter((user) => user.role === "event_coordinator" && !user.isBanned)
        );
      } catch (err) {
    ...
    >
      <form id="add-event-form" onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Input label="Event Name" name="name" type="text" placeholder="Enter Event Name" value={form.name} onChange={handleChange("name")} />
          <Input label="Tagline" name="tagline" type="text" placeholder="Enter Tagline" value={form.tagline} onChange={handleChange("tagline")} />

          <div>
            <label className="form-label pl-1">Event Category</label>
            <select className="form-input appearance-none cursor-pointer" value={form.category} onChange={handleChange("category")}>
              <option className="bg-slate-900" value="">-- Select Category --</option>
              <option className="bg-slate-900" value="Technical">Technical</option>
              <option className="bg-slate-900" value="Cultural">Cultural</option>
              <option className="bg-slate-900" value="Gaming">Gaming</option>
              <option className="bg-slate-900" value="Sports">Sports</option>
            </select>
          </div>

          <Input label="Date" name="date" type="date" value={form.date} onChange={handleChange("date")} />
          <Input label="Location" name="location" type="text" placeholder="Enter Location" value={form.location} onChange={handleChange("location")} />
          <Input label="Fees" name="fees" type="number" placeholder="Enter Fees" value={form.fees} onChange={handleChange("fees")} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            >
              ...
              {coordinators.map((coordinator) => (
                <option key={coordinator._id} className="bg-slate-900" value={coordinator._id}>
                  {coordinator.name}
                </option>
              ))}
            </select>
          ...
          <Input label="Student Coordinator Name" name="studentCoordinatorName" type="text" placeholder="Enter Student Coordinator Name" value={form.studentCoordinatorName} onChange={handleChange("studentCoordinatorName")} />
          <Input label="Student Coordinator Email" name="studentCoordinatorEmail" type="email" placeholder="Enter Student Coordinator Email" value={form.studentCoordinatorEmail} onChange={handleChange("studentCoordinatorEmail")} />
          <Input label="Student Coordinator Phone" name="studentCoordinatorPhone" type="text" placeholder="Enter Student Coordinator Phone" value={form.studentCoordinatorPhone} onChange={handleChange("studentCoordinatorPhone")} />

          <div>
            <label className="form-label pl-1">Status</label>
            <select className="form-input appearance-none cursor-pointer" value={form.status} onChange={handleChange("status")}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
          ...
          <Input
            ...
            type="number"
            placeholder={form.participationType === "solo" ? "Enter max players" : "Enter max teams"}
        ...
        <PrizeCards prizes={prizes} setPrizes={setPrizes} />
